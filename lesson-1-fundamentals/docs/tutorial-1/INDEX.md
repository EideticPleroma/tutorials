# Documentation Index

Quick reference to all documentation in Tutorial 1.

## By Reading Order

1. **[README.md](../../../README.md)** - Project overview and quick start
2. **[Software Engineering Evolution](./concepts/software-engineering-evolution.md)** - The changing landscape
3. **[LLM Fundamentals](./concepts/llm-fundamentals.md)** - How LLMs work
4. **[Tool Calling Architecture](./concepts/tool-calling-architecture.md)** - The core mechanism
5. **[MCP Introduction](./concepts/mcp-intro.md)** - Integration layer
6. **[Testing Agents](./concepts/testing-agents.md)** - O.V.E. methodology
6a. **[Architecture in AI Era](./concepts/architecture-in-ai-era.md)** - New architectural mindset
7. **[Prompting Techniques](./guides/prompting.md)** - Writing effective prompts
8. **[Context Management](./guides/context-management.md)** - Managing Cursor IDE context
9. **[Engineering Best Practices](./guides/engineering.md)** - Development workflows
10. **[Agentic Code Practices](./guides/agentic-practices.md)** - Writing tools for agents
11. **[Progress Tracker](../../progress.md)** - Track your learning
12. **[Architecture Evolution](./architecture/evolution.md)** - Advanced: How architecture is changing
13. **[LLM Driven Design](./architecture/llm-driven-design.md)** - Advanced: Prompting for design
14. **[Visualization Tools](./architecture/visualization-tools.md)** - Advanced: Communicating with diagrams

## By Category

### Concepts (Foundation)
- [Software Engineering Evolution](./concepts/software-engineering-evolution.md)
- [LLM Fundamentals](./concepts/llm-fundamentals.md)
- [Tool Calling Architecture](./concepts/tool-calling-architecture.md)
- [MCP Introduction](./concepts/mcp-intro.md)
- [Tech Stack Decisions](./concepts/tech-stack-decisions.md) - Reference
- [Testing Agents](./concepts/testing-agents.md)
- [Architecture in AI Era](./concepts/architecture-in-ai-era.md)

### Guides (Practical)
- [Prompting Techniques](./guides/prompting.md)
- [Context Management](./guides/context-management.md)
- [Engineering Best Practices](./guides/engineering.md)
- [Agentic Code Practices](./guides/agentic-practices.md)

### Architecture (Advanced)
- [Evolution of Architecture](./architecture/evolution.md)
- [LLM Driven Design](./architecture/llm-driven-design.md)
- [Visualization Tools](./architecture/visualization-tools.md)

### Reference
- [Reading Guide](./READING_GUIDE.md) - Start here for reading order
- [README.md](../../../README.md) - Project overview
- [Progress Tracker](../../progress.md) - Learning checklist

## Quick Lookup

**I want to understand...**
- How LLMs work → [LLM Fundamentals](./concepts/llm-fundamentals.md)
- How tool calling works → [Tool Calling Architecture](./concepts/tool-calling-architecture.md)
- How to write prompts → [Prompting Techniques](./guides/prompting.md)
- How to test agents → [Testing Agents](./concepts/testing-agents.md)
- How to use Cursor → [Context Management](./guides/context-management.md)
- How to write tools → [Agentic Code Practices](./guides/agentic-practices.md)
- Why things are changing → [Software Engineering Evolution](./concepts/software-engineering-evolution.md)
- How to do architecture with AI → [Architecture Guides](./architecture/llm-driven-design.md)

**I need to...**
- Get started → [README.md](../../../README.md)
- Know what to read next → [Reading Guide](./READING_GUIDE.md)
- Track my progress → [Progress Tracker](../../progress.md)
