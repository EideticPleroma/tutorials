...
def check_ollama():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(
                "Ollama server running", False, f"Status code: {response.status_code}"
            )
            return False
    except ImportError:
        print_status("httpx library", False, "Run: pip install httpx")
        return False
    except Exception as e:  # noqa: BLE001 - Diagnostic tool needs broad catching
...
def check_model():
    ...
    try:
        ...
        for model in models["models"]:
            # Model objects have .model and .size attributes
            model_name = model.model if hasattr(model, "model") else "unknown"
            size = (
                model.size / (1024**3) if hasattr(model, "size") else 0
            )  # Convert to GB
        ...
        llama_models = [
            ...
            if hasattr(m, "model") and "llama3.1" in m.model.lower()
        ]

        if llama_models:
            print_status("llama3.1 model available", True, llama_models[0].model)
            return True
        else:
...
def show_registered_tools():
    ...
    try:
        from src.agent.tool_registry import registry  # noqa: E402
        from src.agent import simple_agent  # noqa: E402, F401 - side-effect import
...
def show_configuration():
    ...
    try:
        from src.agent.agent_config import config  # noqa: E402
...
def test_agent(query: str):
    ...
    try:
        from src.agent.simple_agent import Agent  # noqa: E402
...
def main():
    ...
    parser.add_argument(
        "--test", type=str, metavar="QUERY", help="Test agent with a query"
    )
